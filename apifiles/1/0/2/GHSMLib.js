...
!function (window, document) {
    ...
    function UserInfo(cardId) {
        ...
        var isInWhereCommunity = function (street) {
            var point = {
                lat: ui.location.lat,
                lng: ui.location.lng
            };
            ...
            for (var key in communitys) {
                ...
                if (flag) {
                    ...
                    ui.community.id3 = community.id3 || "";
        ...
        var isInWhereStreet = function (streets) {
            var point = {
                lat: ui.location.lat,
                lng: ui.location.lng
            };
            ...
            for (var key in streets) {
                ...
                var poly = street.poly;

                flag = isInsidePolygon(point, poly);
                if (flag) {
                    $.getJSON(APIUrl[0] + APIUrl[4].replace("$street", street.file), function (data) {
                        isInWhereCommunity(data);
                    });
                    ui.street = street.id;
                    ui.streetName = street.name;
                    break;
                }
            }
        ...
        var getPolygon = function () {
            $.getJSON(APIUrl[0] + APIUrl[4].replace("$street", "street"), function (data) {
                isInWhereStreet(data);
            });
        };
